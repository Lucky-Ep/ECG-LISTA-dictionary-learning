...
def data_rand_slice(X: np.ndarray, n_ecg: int, n_slice: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    slice_train_idx = np.array([]).astype(int)
    slice_test_idx = np.array([]).astype(int)
    for i in range(n_slice):
        slice_train_idx = np.concatenate([slice_train_idx, (ecg_train_idx + i)])
        slice_test_idx = np.concatenate([slice_test_idx, (ecg_test_idx + i)])
